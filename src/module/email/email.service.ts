...
import { EmailSubscription, EmailSubscriptionSchema} from './entities/email.entity';
...
export class EmailService {
  ...
  constructor(@InjectModel(EmailSubscription.name) private readonly emailModel: Model<EmailSubscription>,
  ...
  async create(createEmailDto: CreateEmailDto) {
    ...
    if(!user){
      throw new Error(`The user with the ID ${userId} wasn't found please try again`)
    }
  ...
  async remove(apikey:string) {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3000/user/remove', subscription)
      ...
      console.error(`There is a problem with the user remove by apikey ${error}`)
    }
  }
}
